...
from base_data_two_photo import f_trial3, trial3_stim_index
...
if __name__ == '__main__':
    set_seed(16, True)
    # interpolated, new_index = direct_interpolation(f_trial3, trial3_stim_index, 10)
    # f_trial3 = interpolated
    # trial3_stim_index = new_index

    sel_thr = 100
    f_test_sum = np.sum(f_trial3, axis=-1)
    selected_index = np.where(f_test_sum > sel_thr)[0]
    f_selected = f_trial3[selected_index]
    ...
    clus_res = hierarchy.fcluster(Z, t=14, criterion='distance') - 1  # 15 in ward mode
    ...
    firing_curve_cluster_config['multi_stim_index'] = trial3_stim_index

...
if __name__ == '__main__':
    ...
    blank = np.zeros(shape=(len(interpolated_ave), 10))
    inter_all = np.concatenate([interpolated1, blank, interpolated2, blank, interpolated3, blank, interpolated4, blank, interpolated5], axis=1)
    index_all = np.concatenate([new_index, new_index + 800, new_index + 1600, new_index + 2400, new_index + 3200], axis=1)
    f_trial1 = inter_all
    trial1_stim_index = index_all
    ...
    clus_res = hierarchy.fcluster(Z, t=12, criterion='distance') - 1  # 15 in ward mode
